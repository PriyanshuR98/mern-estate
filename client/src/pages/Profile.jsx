...
//   const handleFileUpload = (file) =>{
//     // app was created in firebase.js
//     const storage= getStorage(app);
//     const fileName= new Date().getTime()+ file.name;
//     const storageRef= ref( storage , fileName)
//     const uploadTask= uploadBytesResumable(storageRef,file); 
...
//       res= await fetch(`/api/user/listings/${currentUser._id}`);
//       const data= res.json();
...
//         <p className='self-center text-sm'>
//           {fileUploadError ? (
//           <span className='text-red-700'>Error Image upload(image must be smaller than 2mb)</span>  
...
//       <p className='mt-5 text-red-700' >{ updateSuccess ? 'user udpated successfully! ' : '' }</p>  
      
//       <button onClick={handleShowListings} className='w-full text-green-700'> Show Listings</button>
//       <p className='mt-5 text-sm'>{showListingserror ? 'Error showing listings':''}</p>
